...
#read in file####
readProfileAll<-read_csv(file=paste0(dirPath,"/",Level1_files), col_types = cols()) #last argument suppresses the message on input about column types, helpful for mass upload
...
for(fileIndex in 1:nrow(Level1_files_log)){
  ...
  #get a read profile for each individual profile####
  readProfile1<-readProfileAll[readProfileAll$fileName==Level1_files_log$Level0_profiles[fileIndex],]
